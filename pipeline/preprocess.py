...
def dewpoint_to_humidity(t_c: pd.Series, td_c: pd.Series) -> pd.Series:
    ...
    # Ensure result is a pandas Series (operations with numpy may return ndarray)
    rh_values = 100 * numerator / denominator
    rh = pd.Series(rh_values, index=t_c.index)
...
def compute_wind_speed(u: pd.Series, v: pd.Series) -> pd.Series:
    ...
    return pd.Series(speed, index=u.index).round(4)
